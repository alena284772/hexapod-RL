...
class SimHandler:
    ...
    def __init__(self, xml, xml_path, simlength, simout = None, timestep=None, actuator_callback=None, recorder=None):
        """
        ...
        self.simout = simout
        if isinstance(self.simout, tuple):
            for so in self.simout:
                if so:
                    so.prepare(self.model, self.data, self.n_steps)
        else:
            if self.simout:
...
class PosVelOutput(SimOutput):
    """A class for collecting selected indexes of qpos and qvel."""
    def __init__(self, qpos_idxs, qvel_idxs):
        self.qpos_idxs = qpos_idxs
        self.qvel_idxs = qvel_idxs

    def prepare(self, model, data, n_steps):
        self.model = model
        self.data = data
        self.times = np.full(n_steps, None)
        self.pos = np.full([n_steps, len(self.qpos_idxs)], None)
        self.vel = np.full([n_steps, len(self.qvel_idxs)], None)

    def update(self, step):
        self.pos[step] = self.data.qpos[self.qpos_idxs].data
        self.vel[step] = self.data.qvel[self.qvel_idxs].data
        self.times[step] = self.data.time
